...
class CliEndToEndTests(unittest.TestCase):
    ...
    def test_setup_index_query_refresh_export_uninstall(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            compact = self.run_cli("query", "--workspace", str(root), "current_value", "--compact")
            self.assertIn("evidence", compact)
            self.assertNotIn("diagnostics", compact)
            self.assertNotIn("plan", compact)
            self.assertTrue(compact["evidence"]["items"])
            item = compact["evidence"]["items"][0]
            self.assertEqual(list(item.keys()), ["path", "line", "code"])
            route_compact = self.run_cli("skills", "route", "current_value", "--workspace", str(root), "--compact")
            self.assertIn("route_id", route_compact)
            self.assertIn("recommendations", route_compact)
            self.assertNotIn("learning", route_compact)
