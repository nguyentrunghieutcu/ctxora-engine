...
test("compares stable release versions deterministically", () => {
  assert.equal(compareVersions("6.4.0", "6.5.3"), -1);
  assert.equal(compareVersions("6.5.3", "6.5.3"), 0);
  assert.equal(compareVersions("7.0.0", "6.5.3"), 1);
  assert.equal(compareVersions("latest", "6.5.3"), null);
});
...
test("reports the npm package version without installing Python", () => {
  ...
  assert.equal(result.stdout.trim(), "6.5.4");
});
