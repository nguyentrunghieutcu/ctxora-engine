...
def _compact_openai(
    ...
    from harness_context.infrastructure.compact.openai import compact_openai_messages
...
def _compact_anthropic(messages: list[dict], retain_turns: int) -> list[dict]:
    ...
    """
    from harness_context.infrastructure.compact.anthropic import summarize_anthropic_history
...
def _compact_gemini(messages: list[dict], retain_turns: int) -> list[dict]:
    ...
    """
    from harness_context.infrastructure.compact.gemini import summarize_gemini_history
    retain_turns = max(0, int(retain_turns))
    retain_msgs = retain_turns * 2
    if len(messages) <= retain_msgs:
        return messages
